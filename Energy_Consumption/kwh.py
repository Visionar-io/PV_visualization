# kwh.py — hourly kWh from counters in S3
...
__all__ = [
    "hourly_kwh_from_counters",
    "diagnose_export_sources",
    "get_data_energy_consumption",
]
...
def key_for_local_date_csv(date_str_local: str):
    ...
    dt = datetime.fromisoformat(str(date_str_local))
    y = dt.strftime("%Y")
    m = dt.strftime("%m")
    d = dt.strftime("%d")
...
def hourly_kwh_from_counters(key: str, start_ms: int, end_ms: int):
    ...
    exp_series = None
    if imports:
        imp_series = np.nansum(
            np.vstack([hourly_cols.get(n, np.full(len(hours_sorted), np.nan)) for n in imports]),
            axis=0,
        )
    if exports:
        exp_series = np.nansum(
            np.vstack([hourly_cols.get(n, np.full(len(hours_sorted), np.nan)) for n in exports]),
            axis=0,
        )
    ...
    for ph in phases:
        out_header.append(
            f"{ph.replace('_Total_kWh','')}_kWh" if ph.endswith("_Total_kWh") else f"{ph}_kWh"
        )
    if phases:
    ...
    return rows, out_header, text

# -------- Diagnose which phase caused Export --------
def diagnose_export_sources(csv_text: str, start_ms: int, end_ms: int):
    ...
    imp_energy = {ph: 0.0 for ph in phases}
    exp_energy = {ph: 0.0 for ph in phases}
    ...
    for ph in phases:
        ...
        def _fmt(x):
            return f"{x:10.3f}" if (x == x) else f"{'':>10}"

        print(f"{ph:<4} {neg_ratio:7.2f}  {_fmt(imp_p)} {_fmt(exp_p)}  {_fmt(imp_m)} {_fmt(exp_m)}   {verdict}")

# -------- Public API: get_data_energy_consumption --------
def get_data_energy_consumption(DATE):
    """
    High-level helper to:
      - compute hourly kWh for a given local date
      - save CSV locally
      - run diagnose_export_sources
    DATE can be:
      - str "YYYY-MM-DD"
      - datetime.date
    """
...
def main():
    ...
    get_data_energy_consumption(date_local)
